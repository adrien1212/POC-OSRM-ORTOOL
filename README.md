...
```bash
cd backend
OSRM_BASE_URL=http://127.0.0.1:5001 ./mvnw spring-boot:run
...
cp .env.example .env
bun install
bun dev              # http://localhost:3000, proxies /api and /osrm
bun run lint
```

`bun.lock` is authoritative — the frontend does not use npm.

## Deploying

Pushing a git **tag** triggers [`.github/workflows/deploy.yml`](./.github/workflows/deploy.yml): it
builds multi-arch images, pushes them to GHCR, and runs
[`docker-compose.prod.yml`](./docker-compose.prod.yml) on the target host. Configure it with the
repository variables `VPS_HOST`, `VPS_USER`, `APP_DIR` and the secret `DEPLOY_KEY_VPS`.

On the host, set `APP_CORS_ALLOWED_ORIGINS` to your own domain — the backend defaults to localhost
only.

## Contributing

Bug reports and pull requests welcome — see [CONTRIBUTING.md](./CONTRIBUTING.md) and the
